...
class EmailService {
  constructor() {
    this.fromAddress = process.env.EMAIL_USER || 'noreply@royalEstate.com';
  }
  ...
  async sendEmail(to, subject, htmlContent, options = {}) {
    try {
      ...
      console.warn(`⚠️ Email not sent: ${subject} to ${to}. Reason: ${error.message}`);
  ...
  async sendWelcomeEmail(userEmail, userName) {
    const subject = 'Welcome to RoyalEstate - Your Account is Ready!';
  ...
  async sendEmailVerification(userEmail, userName, verificationUrl) {
    const subject = 'Verify Your RoyalEstate Email Address';
  ...
  async sendPasswordResetEmail(userEmail, resetUrl) {
    const subject = 'Reset Your RoyalEstate Password';
  ...
  async sendNewsletterWelcome(userEmail) {
    const subject = 'Welcome to RoyalEstate Newsletter 📧';
  ...
  async sendAppointmentNotificationToAdmin(appointment, userDetails) {
    ...
    const adminEmail = process.env.ADMIN_EMAIL || 'admin@royalEstate.com';
  ...
  async sendUserSuspended(userEmail, userName, days, reason, suspendedUntil) {
    const subject = `RoyalEstate Account Suspended - ${days} Day${days > 1 ? 's' : ''}`;
  ...
  async sendUserBanned(userEmail, userName, reason) {
    const subject = 'RoyalEstate Account Permanently Banned';
  ...
  async sendUserReactivated(userEmail, userName) {
    const subject = 'RoyalEstate Account Reactivated - Welcome Back!';
